...
- **Scalable Architecture**: Built on YCCP v4 modular engineering standards.
...
| **Standard** | **YCCP v4 Engine** | Internal engineering standard for scalable projects |
...
└── frontend/
    ├── app/
    │   ├── globals.css      # Tailwind v4 全域配置與自定義樣式
    │   ├── layout.tsx       # 根佈局 (HTML 地基與 SEO Metadata)
    │   └── page.tsx         # 核心頁面內容 (YCCP 響應式佈局邏輯)
    ├── public/
    │   └── me.jpg           # 個人形象照片 (Portrait)
    ├── .gitignore           # Git 忽略清單 (由 YCCP 自動配置)
    ├── package.json         # 專案套件依賴管理
    └── tsconfig.json        # TypeScript 強編譯設定
🚀 Quick Start
1. Prerequisites
...
Change Footer Signature: Search for the string YCCP to customize system credits.
